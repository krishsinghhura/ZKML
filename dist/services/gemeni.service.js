...
exports.generateFlaskApp = generateFlaskApp;
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
const generative_ai_1 = require("@google/generative-ai");
async function generateFlaskApp(params) {
    const { folderPath, modelFileName, requirementsFileName, modelInputs, modelDescription, gemeniModelName = "gemini-2.5-flash", } = params;
...
`;
    ...
    try {
        ...
        if (!parsed["app.py"])
            throw new Error("Missing 'app.py' key in Gemeni response");
    ...
    const appPyPath = path_1.default.join(folderPath, "app.py");
    await fs_extra_1.default.writeFile(appPyPath, appCode);
    // Append flask to requirements.txt
    const reqPath = path_1.default.join(folderPath, requirementsFileName);
    await fs_extra_1.default.ensureFile(reqPath);
    const existingReq = await fs_extra_1.default.readFile(reqPath, "utf-8");
    const deps = existingReq.split("\n").map(l => l.trim()).filter(Boolean);
    if (!deps.includes("flask"))
        deps.push("flask");
    await fs_extra_1.default.writeFile(reqPath, deps.join("\n"));
